...
import ProductFeaturesInput from "../../components/input/ProductFeaturesInput";
toast.configure({
  autoClose: 2000,
  draggable: false,
  limit: 3,
...
function ProductActions(props) {
  ...
  const handleTagsChange = (newFeatures) => {
    setTags(newFeatures);
  };
  ...
  const handleRemoveAllImage = () => {
    ...
    Promise.all(promises)
      .then(() => {
        setProgressListUpload(0);
        setUrlsListImage([]);
        toast.success("Xoá hình ảnh thành công!");
      })
      .catch((error) => {
        toast.warning("Xoá hình ảnh không thành công!");
        console.log(error);
      });
  };
  ...
  useEffect(() => {
    ...
    if (id) {
      getOneItem(id).then((res) => {
        setTags(res.data?.features?.split(","));
        setProduct(res.data);
          ...
          <form onSubmit={handleSubmit}>
            <div className="row">
              <div className="col-12">
                    ...
                    <div className="row">
                      ...
                      <div className="col-12">
                        <ProductFeaturesInput
                          features={tags} // Truyền tags vào component con
                          onChange={handleTagsChange} // Truyền hàm xử lý onChange
                        />
                      </div>
                ...
                <div className="card">
                  <div className="glOjBk list-cusomer-order">
                    <h2 className="heading">Thông tin thêm</h2>
                    <p style={{ color: "red" }}>
                      {product.category === ""
